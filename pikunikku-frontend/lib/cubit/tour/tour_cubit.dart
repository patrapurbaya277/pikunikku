...
class TourCubit extends Cubit<TourState> {
  ...
  void resetSingle() {
    emit(state.resetSingle());
  }

  void getAllTour(context) async {
  ...
  void getTourDetail(BuildContext context, Tour tour) {
    ...
    avoider.catchException(
      ...
      function: () async {
        http.Response response;
        response = await http.get(API.singleTour(tour.slug.toString()));
        
        final jsonData = jsonDecode((response.body).toString());
        if (response.statusCode == 200) {
          Tour _tour = Tour.fromJson(jsonData["data"]);
          emit(state.copyWith(singleTour: _tour));
          getListPaket(context);
  ...
  void updateDetail(BuildContext context, Tour tour) {
    Avoider avoider = Avoider();
    avoider.catchException(
      context: context,
      function: () async {
  ...
  void getListPaket(context) async {
    Avoider avoider = Avoider();
    avoider.catchException(
      functionBreak: () {
        emit(state.copyWith(loadingPaket: false));
      },
      context: context,
      function: () async {
        emit(state.copyWith(
        ...
        if (response.statusCode == 200) {
          ...
          emit(state.copyWith(
            listPaket: _listPaket,
          ));
          List<Paket> _listSelectedPaket = List.from(_listPaket.where(
              (element) => element.configPaketId == state.singleTour!.id));
          if (_listSelectedPaket.length != 0) {
            emit(state.copyWith(
                hasFilteredData: true, selectedListPaket: _listSelectedPaket));
          }
          // emit(state.copyWith(loadingData: false));
  ...
  void showFilterPaket(context) {
    Navigator.of(context).push(
        MaterialPageRoute(builder: (BuildContext context) => PaketPage()));
  }
}
